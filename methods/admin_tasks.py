...
class Method:  # pylint: disable=E1101,R0903,W0201
    ...
    @web.method()
    def migrate_eval_dataset_columns(self, *args, **kwargs):
        ...
        Param format (optional):
            "project_id=<all|N>"
        """
        from ..utils.eval_dataset_schema import apply_eval_dataset_columns
            ...
            if seg.lower().startswith("project_id="):
                ...
                if value.lower() != "all":
                    try:
                        ...
                        log.warning(
                            "migrate_eval_dataset_columns: invalid project_id '%s', scanning all", value)
        ...
        try:
            ...
            log.exception("migrate_eval_dataset_columns: failed to list projects")
            return {"migrated": 0, "error": "failed to list projects"}

        migrated, failed = apply_eval_dataset_columns(project_ids)
        return {"migrated": len(migrated), "failed": len(failed), "failed_projects": failed}

    @web.method()
    def migrate_eval_suite_columns(self, *args, **kwargs):
        """Admin task: add the ``description`` column to ``eval_suite`` on each
        project schema. Idempotent (``ADD COLUMN IF NOT EXISTS``): safe to run
        multiple times.

        Skip on fresh environments: ``create_all`` already provisions this column
        from the ORM model, so a brand-new deployment has nothing to migrate. Only
        run this on environments that already had ``eval_suite`` tables before this
        column was added (i.e. upgrading an existing deployment).

        Param format (optional):
            "project_id=<all|N>"
        """
        from ..utils.eval_suite_schema import apply_eval_suite_columns
            ...
            if seg.lower().startswith("project_id="):
                ...
                if value.lower() != "all":
                    try:
                        ...
                        log.warning(
                            "migrate_eval_suite_columns: invalid project_id '%s', scanning all", value)
        ...
        try:
            ...
            log.exception("migrate_eval_suite_columns: failed to list projects")
            return {"migrated": 0, "error": "failed to list projects"}

        migrated, failed = apply_eval_suite_columns(project_ids)
        return {"migrated": len(migrated), "failed": len(failed), "failed_projects": failed}
    ...
    def migrate_eval_dimension_columns(self, *args, **kwargs):
        ...
        Param format (optional):
            "project_id=<all|N>"
        """
        from ..utils.eval_dimension_schema import apply_eval_dimension_columns
            ...
            if seg.lower().startswith("project_id="):
                ...
                if value.lower() != "all":
                    try:
                        ...
                        log.warning(
                            "migrate_eval_dimension_columns: invalid project_id '%s', scanning all", value)
        ...
        try:
            ...
            log.exception("migrate_eval_dimension_columns: failed to list projects")
            return {"migrated": 0, "error": "failed to list projects"}

        migrated, failed = apply_eval_dimension_columns(project_ids)
        return {"migrated": len(migrated), "failed": len(failed), "failed_projects": failed}
