...
class PromptLibAPI(api_tools.APIModeHandler):
    ...
    def post(self, project_id: int, **kwargs):
        ...
        # Security check: Block creation of blocked toolkit types
        if is_toolkit_blocked(tool_data.type):
            log.warning(f"[SECURITY] Blocked attempt to create toolkit of type '{tool_data.type}'")
            return {
                "ok": False,
                "error": f"Toolkit type '{tool_data.type}' is not available in this deployment"
            }, 403
