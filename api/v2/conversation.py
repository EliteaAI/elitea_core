...
class PromptLibAPI(api_tools.APIModeHandler):
    ...
    def put(self, project_id: int, conversation_id: int):
        ...
        try:
            parsed = ConversationUpdate.model_validate(data)
            with db.get_session(project_id) as session:
                ...
                if parsed.is_private is not None:
                ...
                update_data = parsed.model_dump(exclude_unset=True, exclude={'is_hidden', 'meta'})
                if update_data:
                    conversation_q.update(update_data)

                # Handle meta field separately to merge instead of replace
                if parsed.meta is not None:
                    conversation.meta = conversation.meta or {}
                    conversation.meta.update(parsed.meta)
